(function (root) {
    ...
    function list() {
        return B.list().map(label);
    }
    function key(id = active) {
        return B.storageKeyFor(id);
    }
    function init() {
        let entries = B.list();
        if (!entries.length) {
            ...
            const id = crypto.randomUUID(),
                raw = B.legacy.get(B.legacy.state);
            const s = raw
                ? StatementEngine.validate(JSON.parse(raw))
                : StatementEngine.fresh(false);
            B.write(id, JSON.stringify(s));
            const undo = B.legacy.get(B.legacy.undo);
            if (undo) B.writeUndo(id, undo);
            entries = B.list();
        }
        let preferred = '';
        try {
            preferred = sessionStorage.getItem('hoistx-active-company');
        } catch {}
    ...
    function select(id) {
        ...
        if (entry.corrupt)
            throw Error(
                'This company file is damaged and cannot be opened. Export or delete it, then restore from a backup.',
            );
        const s = StatementEngine.validate(JSON.parse(B.read(id)));
        active = id;
        blocked = false;
        try {
            sessionStorage.setItem('hoistx-active-company', id);
        } catch {}
        return s;
    }
    function write(s) {
    ...
    function create(name) {
        const s = StatementEngine.fresh(false);
        s.company.name = name;
        const id = crypto.randomUUID();
        B.write(id, JSON.stringify(s));
        return select(id);
