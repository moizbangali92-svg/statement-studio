...
function legacyChecks() {
    ...
    for (const p of periods()) {
        const c = E.calc(state, p),
            label = periodLabel(p);
        out.push({
            ok: Math.abs(c.balance) < 0.005,
            title: `Financial position · ${label}`,
            detail: `Assets less liabilities and equity: ${money(c.balance)}.`,
        });
        out.push({
            ok: Math.abs(c.cashDifference) < 0.005,
            title: `Cash flow reconciliation · ${label}`,
            detail: `Closing cash less financial position cash: ${money(c.cashDifference)}.`,
        });
        out.push({
            ok: c.equityDifferences.every((x) => Math.abs(x.value) < 0.005),
            title: `Equity reconciliation · ${label}`,
            detail:
                c.equityDifferences
                    .filter((x) => Math.abs(x.value) >= 0.005)
                    .map((x) => `${E.groups[x.key]}: ${money(x.value)}`)
                    .join('; ') || 'All four equity components reconcile.',
        });
    }
    if (state.company.comparative) {
        let current = state.equity.current,
            prev = E.calc(state, 'prior');
        out.push({
            ok: Math.abs(state.cash.current.opening - prev.sum('cash')) < 0.005,
            title: 'Opening cash continuity',
            detail: 'Current opening cash must match comparative closing cash.',
        });
        out.push({
            ok: ['Capital', 'Reserve', 'Retained', 'Owner'].every(
                (k) => Math.abs(current['open' + k] - prev.sum(k.toLowerCase())) < 0.005,
            ),
            title: 'Opening equity continuity',
            detail: 'Current opening equity components must match comparative closing balances.',
        });
    }
    out.push({
        ok: !!state.company.name.trim(),
        title: 'Company name',
        detail: 'Enter the registered company name.',
    });
    let c = state.company;
    const validDate = (d) =>
        /^\d{4}-\d{2}-\d{2}$/.test(d) &&
    ...
    out.push({
        ok:
            ...
            c.start <= c.end &&
            (!c.comparative ||
                (validDate(c.priorStart) &&
                    validDate(c.priorEnd) &&
                    c.priorStart <= c.priorEnd &&
                    c.priorEnd < c.start)),
        title: 'Reporting periods',
        detail: 'Use valid dates; the comparative period must end before the current period begins.',
    });
    out.push({
        ok: state.rows.every(
            (r) => r.label.trim() && (!r.note || state.notes.some((n) => n.id === r.note)),
        ),
        title: 'Line descriptions and note references',
        detail: 'Every line needs a description and any linked note must exist.',
    });
    out.push({
        ok:
            ...
            state.notes.every(
                (n) =>
                    n.title.trim() &&
                    (n.autoGroup
                        ? ComparisonNotes.detail(state, n).reviewed &&
                          (!ComparisonNotes.detail(state, n).needsExplanation || n.body.trim())
                        : n.body.trim()),
            ),
        title: 'Notes completed',
        detail: 'Complete manual disclosures and review automatic notes against the latest figures. Add explanations for flagged movements.',
    });
    out.push({
        ok: !!state.management.trim(),
        title: 'Management report',
        detail: 'Add the management report in Company details.',
    });
    return out;
}
function checks() {
...
function checksView() {
    let list = checks(),
        n = list.filter((c) => !c.ok).length;
    return (
        title(
            '04 / REVIEW',
            'Review checks',
            n
                ? `${n} items need attention before the report is ready for review.`
                : 'Arithmetic and completion checks passed. Review the accounting treatment separately.',
        ) +
        `<div class="checks">${list.map((c) => `<section class="check ${c.ok ? 'pass' : 'fail'}"><span>${c.ok ? '✓' : '!'}</span><div><h3>${c.title}</h3><p>${esc(c.detail)}</p></div><b>${c.ok ? 'Passed' : 'Review'}</b></section>`).join('')}</div><p class="muted">These checks test arithmetic, period continuity and completion. They are not an audit or a financial reporting compliance assessment.</p>`
    );
}
...
function reportRows(groups) {
    return groups.flatMap((g) =>
        state.rows
            .filter((r) => r.group === g)
            .map((r) => [r.label, (p) => r[p], false, noteNo(r.note)]),
    );
}
function legacyReportHtml() {
    let c = state.company,
        seq = 0;
    const period = `For the period ${date(c.start)} to ${date(c.end)}`,
        names = [
            'Company information and management report',
    ...
    const sheet = (heading, body) =>
        `<article class="paper ${esc(c.style)}"><div class="paper-head"><strong>${esc(c.name || 'Company name not entered')}</strong><span>DRAFT · UNAUDITED</span></div><h2>${esc(heading)}</h2><p class="period">${esc(period)}</p>${body}<div class="paper-foot"><span>The accompanying notes form part of these draft financial statements.</span><span>${++seq}</span></div></article>`;
    const income = [
        ...reportRows(['revenue', 'cost']),
        ['Gross profit / (loss)', (p) => E.calc(state, p).gross, true],
        ...reportRows(['otherIncome', 'expense', 'finance', 'tax']),
        ['Profit / (loss) for the period', (p) => E.calc(state, p).profit, true],
        ...reportRows(['oci']),
        ['Total comprehensive income / (loss)', (p) => E.calc(state, p).comprehensive, true],
    ];
    const position = [
        { heading: 'ASSETS' },
        { heading: 'Non-current assets' },
        ...reportRows(['noncurrentAsset']),
        ['Total non-current assets', (p) => E.calc(state, p).sum('noncurrentAsset'), true],
        { heading: 'Current assets' },
        ...reportRows(['currentAsset', 'cash']),
        [
            'Total current assets',
            (p) => E.add([E.calc(state, p).sum('currentAsset'), E.calc(state, p).sum('cash')]),
            true,
        ],
        ['TOTAL ASSETS', (p) => E.calc(state, p).assets, true],
        { heading: 'EQUITY AND LIABILITIES' },
        { heading: 'Equity' },
        ...reportRows(['capital', 'reserve', 'retained', 'owner']),
        ['Total equity', (p) => E.calc(state, p).equity, true],
        { heading: 'Non-current liabilities' },
        ...reportRows(['noncurrentLiability']),
        ['Total non-current liabilities', (p) => E.calc(state, p).sum('noncurrentLiability'), true],
        { heading: 'Current liabilities' },
        ...reportRows(['currentLiability']),
        ['Total current liabilities', (p) => E.calc(state, p).sum('currentLiability'), true],
        [
            'TOTAL EQUITY AND LIABILITIES',
            (p) => E.add([E.calc(state, p).liabilities, E.calc(state, p).equity]),
            true,
        ],
    ];
    const cf = [
        { heading: 'Cash flows from operating activities' },
        ['Profit / (loss) for the period', (p) => E.calc(state, p).profit],
        ...['noncash', 'working', 'operatingOther'].map((k) => [
            E.cashFields[k],
            (p) => state.cash[p][k],
        ]),
        ['Net cash from / (used in) operating activities', (p) => E.calc(state, p).operating, true],
        { heading: 'Cash flows from investing activities' },
        ['Net cash from / (used in) investing activities', (p) => state.cash[p].investing, true],
        { heading: 'Cash flows from financing activities' },
        ['Net cash from / (used in) financing activities', (p) => state.cash[p].financing, true],
        ['Exchange effect on cash', (p) => state.cash[p].fx],
        ['Net change including exchange effect', (p) => E.calc(state, p).change, true],
        ['Opening cash and cash equivalents', (p) => state.cash[p].opening],
        ['Closing cash and cash equivalents', (p) => E.calc(state, p).closingCash, true],
    ];
    const eq = periods()
        .slice()
        .reverse()
        .map((p) => {
            const e = state.equity[p],
                cc = E.calc(state, p),
                rows = [
                    ['Opening balance', e.openCapital, e.openReserve, e.openRetained, e.openOwner],
                    ['Profit / (loss)', 0, 0, cc.profit, 0],
                    ['Other comprehensive income', 0, cc.sum('oci'), 0, 0],
                    ['Capital introduced / (repaid)', e.capitalMovement, 0, 0, 0],
                    ['Transfers to reserves', 0, e.reserveTransfer, -e.reserveTransfer, 0],
                    ['Dividends', 0, 0, -e.dividends, 0],
                    ['Shareholders’ account movement', 0, 0, 0, e.ownerMovement],
                    ['Other retained earnings adjustments', 0, 0, e.otherRetained, 0],
                    [
                        'Closing balance',
                        cc.close.capital,
                        cc.close.reserve,
            ...
            return `<h3>Period ended ${esc(date(periodLabel(p)))}</h3><div class="table-scroll"><table class="financial equity-table"><thead><tr>${['Movement', 'Share capital', 'Reserves', 'Retained earnings', 'Shareholders’ account', 'Total'].map((v) => `<th>${v}</th>`).join('')}</tr></thead><tbody>${rows
                .map(
                    (r, i) =>
                        `<tr class="${i === 8 ? 'total' : ''}"><td>${r[0]}</td>${r
                            .slice(1)
                            .map((v) => `<td class="numeric">${money(v)}</td>`)
                            .join('')}<td class="numeric">${money(E.add(r.slice(1)))}</td></tr>`,
                )
                .join('')}</tbody></table></div>`;
        })
        .join('');
    const warnings = checks().filter((x) => !x.ok);
    return (
        `<article class="paper cover ${esc(c.style)}"><div class="paper-head"><span>FINANCIAL STATEMENTS</span><span>DRAFT · UNAUDITED</span></div><div class="cover-title"><div class="cover-rule"></div><h1>${esc(c.name || 'Company name not entered')}</h1><h2>Financial statements</h2><p>${esc(period)}</p><p>Presented in ${esc(c.currency)}</p>${c.comparative ? `<p class="muted">Comparative period: ${esc(date(c.priorStart))} to ${esc(date(c.priorEnd))}</p>` : ''}</div><p class="cover-status">Prepared for review${state.demo ? ' · ILLUSTRATIVE DATA' : ''}</p><p>This pack is prepared from user-entered information. It contains no independent auditor’s opinion.</p>${warnings.length ? `<div class="report-warning">${warnings.length} preparation checks require attention. See the review summary below.</div>` : ''}</article>` +
        sheet(
            'Contents',
            `<ol class="contents">${names.map((n, i) => `<li><span>${n}</span><b>${i + 2}</b></li>`).join('')}</ol><p class="muted">Numbers refer to report sections. Long disclosures may continue onto additional printed pages.</p>${warnings.length ? `<h3>Preparation review summary</h3><ul>${warnings.map((w) => `<li>${esc(w.title)} — ${esc(w.detail)}</li>`).join('')}</ul>` : ''}`,
        ) +
        sheet(
            names[0],
            `<dl>${[
                ['Registered name', c.name],
                ['Registration / licence', c.registration],
                ['Registered address', c.address],
                ['Principal activity', c.activity],
                ['Manager / authorised signatory', c.manager],
            ]
                .map(([k, v]) => `<dt>${k}</dt><dd>${esc(v || 'Not entered')}</dd>`)
                .join(
                    '',
                )}</dl><h3>Management report</h3><div class="prose">${esc(state.management || 'Management report not entered.')}</div>`,
        ) +
        sheet(names[1], financialTable(position, true)) +
        sheet(
            names[2],
            `<p class="muted">Expense lines are shown as positive amounts and deducted in the totals.</p>${financialTable(income, true)}`,
        ) +
        sheet(
            names[3],
            `<p>All amounts in ${esc(c.currency)}. Other comprehensive income is allocated to reserves.</p>${eq}`,
        ) +
        sheet(
            names[4],
            `<p>Indirect method. Outflows are shown in parentheses.</p>${financialTable(cf)}`,
        ) +
        sheet(
            names[5],
            state.notes
                .map(
                    (n, i) =>
                        `<section class="disclosure"><h3>${i + 1}. ${esc(n.title || 'Untitled note')}</h3>${n.autoGroup ? comparisonContent(n) : ''}<div class="prose">${esc(n.body || (n.autoGroup ? '' : 'Disclosure not entered.'))}</div>${!n.autoGroup && state.rows.some((r) => r.note === n.id) ? financialTable(state.rows.filter((r) => r.note === n.id).map((r) => [r.label, (p) => r[p]])) : ''}</section>`,
                )
                .join(''),
        )
    );
...
function legacyReportView() {
    return (
        title(
            '05 / OUTPUT',
            'Report preview',
            'A comparative statement pack, ready to print on A4.',
            `<button data-action="print" class="primary">Print / Save PDF ↗</button>`,
        ) +
        `<div class="hint">Choose “Save as PDF” in the print dialog. Use A4, 100% scale and turn off browser headers and footers.</div><div class="report-stack">${reportHtml()}</div>`
    );
}
function download() {
