...
function localStorageAdapter(ls) {
    return {
        ...
        async read(id) { return ls.getItem(companyKey(id)); },
        async write(id, text) { ls.setItem(companyKey(id), text); },
        async remove(id) { ls.removeItem(companyKey(id)); ls.removeItem(undoKey(id)); },
        async readUndo(id) { return ls.getItem(undoKey(id)); },
        async writeUndo(id, text) { ls.setItem(undoKey(id), text); },
        async clearUndo(id) { ls.removeItem(undoKey(id)); },
        raw: ls,
    };
}
...
function electronAdapter(api) {
    return {
        kind: 'desktop',
        async listIds() { return (await api.list()).map(e => e.id); },
        async read(id) { return api.read(id); },
        async write(id, text) { return api.write(id, text); },
        async remove(id) { return api.remove(id); },
        async readUndo(id) { return api.readUndo(id); },
        async writeUndo(id, text) { return api.writeUndo(id, text); },
        async clearUndo(id) { return api.clearUndo(id); },
        raw: api,
    };
}
...
function createBackend(adapter, options) {
    ...
    function emit(name, detail) {
        ...
        try { root.dispatchEvent(new root.CustomEvent(name, { detail })); } catch (e) { /* non-DOM host */ }
    }

    function markDirty(id) {
        dirty.add(id);
        if (flushTimer !== null) return;
        flushTimer = setTimeout(function () { flushTimer = null; flush(); }, flushMs);
    }
    ...
    const ready = (async () => {
        ...
        for (const id of ids) {
            try {
                ...
                cache.set(id, { text: null, name: '', end: '', corrupt: true, error: String(err.message) });
            }
        }
        hydrated = true;
    ...
    function requireHydrated() {
        if (!hydrated) throw new Error('StoreBackend used before hydration; await StoreBackend.ready first.');
    }
    ...
    return {
        ...
        list() {
            ...
            return [...cache.entries()]
                .map(e => ({ id: e[0], name: e[1].name, end: e[1].end, corrupt: e[1].corrupt, error: e[1].error }))
                .sort((a, b) => (a.name || '￿').localeCompare(b.name || '￿') || a.id.localeCompare(b.id));
        },
        has(id) { requireHydrated(); return cache.has(id); },
        read(id) { requireHydrated(); const c = cache.get(id); return c ? c.text : null; },
        write(id, text) {
            requireHydrated();
            if (typeof text !== 'string') throw new Error('StoreBackend.write expects a JSON string');
            cache.set(id, describe(text));
            markDirty(id);
        },
        async remove(id) {
        ...
        readUndo(id) { return adapter.readUndo(id); },
        writeUndo(id, text) { return adapter.writeUndo(id, text); },
        clearUndo(id) { return adapter.clearUndo(id); },
        ...
        legacy: {
            ...
            get(key) {
                ...
                try { return adapter.raw.getItem(key); } catch (e) { return null; }
            },
        },
...
const factories = {
    createBackend, localStorageAdapter, electronAdapter, memoryStorage,
    companyKey, undoKey, idFromKey,
    COMPANY_PREFIX, UNDO_SUFFIX, LEGACY_INDEX, LEGACY_STATE, LEGACY_UNDO,
};
